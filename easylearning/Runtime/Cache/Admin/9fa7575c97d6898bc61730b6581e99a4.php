						...
						<div class="page-header">
						 <form action="" method="get">
									 ...
									 -->
											...
											<input  type="text" id="factoryname" placeholder="维修厂" name="factoryname" style="height:32px;" value="<?php echo ($_GET['factoryname']); ?>">
											</span>
											<span class="input-icon input-icon-right">
											<input  type="text" id="car_brandname" placeholder="品牌" name="car_brandname" style="height:32px;" value="<?php echo ($_GET['car_brandname']); ?>">
											</span>
											<input  class=" btn-lg btn-success btn btn-xs btn-danger" type="submit" value="搜索">
										</div>
										 <?php if($role == 1): ?><a class="label label-xlg label-primary arrowed-right pull-right " href="<?php echo U('Factorybrand/add');?>">添加维修厂品牌</a><?php endif; ?>
						</form>
												 ...
												 <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
														...
														<?php if($role == 1): ?><td>
															<div class="btn-group">
																...
																<!-- <a href="<?php echo U('Factorybrand/edit',array('factoryid'=>$v['factoryid'],'id'=>$v['factoryid']));?>" class="btn btn-xs btn-info">
																	<i class="icon-edit bigger-120"></i>
																</a> -->

																<a onclick="if(confirm('确认删除?')) location.href='<?php echo U('Factorybrand/del',array('factoryid'=>$v[factoryid],'car_brandid'=>$v[car_brandid]));?>'"  href="javascript:;"  class="btn btn-xs btn-danger">
