    ...
    public function search_courses($searchcriteria) {
        ...
        if (!empty($searchcriteria)) {
            ...
            if (!empty($searchcriteria['search'])) {
                ...
                // holy shit!  jquery in this place not work, switch to curl post
                $curl = curl_init();
                curl_setopt($curl, CURLOPT_URL, "http://xenoeye.org:49999/search/getSimilarCourses?query=".$searchcriteria['search']);
                ...
                if ($res -> status == "0"){
                    ...
                    foreach ($res->data as $record) {
                        ...
                        if(!array_key_exists($record->id,$courses)){
                            $excourses[$record->id] = new course_in_list($record);
                        }
                    }
                    
                    // generate html code to display courses
                    $extotalcount = count($excourses);
                    if ($extotalcount > 0){
                        $content .= $this->heading("Try recommended courses");
                        $excourseslist = $this->coursecat_courses($chelper, $excourses, $extotalcount);
                        
                        $content .= $excourseslist;
                        
                        $content .= $this->box_start('generalbox mdl-align searchexpand');
                        $content .= $this->box_end();
                    }
                }
            }
